...
2. CI validates every team configuration and runs tests.
3. The generator produces Kubernetes manifests under `generated/<team>-<environment>/`.
4. Kubeconform validates all generated manifests against strict Kubernetes schemas.
5. CI fails if committed manifests differ from regenerated output.
6. A platform engineer reviews the pull request, including the generated RBAC, quota, limits, and network policies.
7. The manifests are applied through GitOps or manually with `kubectl`.
8. The team receives namespace-scoped access through the generated RoleBindings.
