...
def validate_config(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "quota": {"cpu": quota_cpu, "memory": quota_memory, "pods": quota_pods},
        "network": {
...
def service_account(config: dict[str, Any]) -> dict[str, Any]:
    return {
        "apiVersion": "v1",
        "kind": "ServiceAccount",
        "metadata": metadata(config, f"{config['team']}-developer"),
        "automountServiceAccountToken": False,
    }


def role(name: str, config: dict[str, Any], verbs: list[str]) -> dict[str, Any]:
    return {
        "apiVersion": "rbac.authorization.k8s.io/v1",
        "kind": "Role",
        "metadata": metadata(config, name),
        "rules": [
            {"apiGroups": [""], "resources": APP_RESOURCES, "verbs": verbs},
...
def rbac(config: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        ...
        role("developer", config, WRITE_VERBS),
...
def files_for(config: dict[str, Any]) -> dict[str, str]:
    return {
        ...
        "01-service-account.yaml": dump(service_account(config)),
